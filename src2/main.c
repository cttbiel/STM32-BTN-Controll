...
uint8_t button_pressed(void) {
	...
	if (HAL_GPIO_ReadPin(BT1_GPIO_Port, BT1_Pin) == GPIO_PIN_RESET) { // Botão pressionado
	        if (button_was_pressed == 0) {
	            HAL_Delay(50); // Delay para debounce
	            button_was_pressed = 1;
	            return 1; // Retorna que o botão foi pressionado
	        }
	    } else {
	        button_was_pressed = 0; // Reset para próxima detecção
	    }
...
int main(void)
{
  ...
  while (1)// Here's the interesting part, this while is changing the status between 0 ( which is off) and 1 ( on)
  {
	  if (button_pressed()) {
		  ...
		  if(led_state>3){
			  led_state = 0; // Reseting to inicial state
		  }
